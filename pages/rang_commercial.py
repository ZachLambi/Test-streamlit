...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Intégration réelle (remplace l'emplacement réservé) d'analyse_rang_commerce_v3.py,
PORTÉE : Québec / État(s) américain(s) seulement. Pays/Pays et Province/Pays
viendront une fois Comtrade réconcilié (voir resume_reconciliation_comtrade.txt).

Deux classements calculés, comme le script original :
  - Rang_vs_provinces         : Québec contre les autres provinces
  - Rang_vs_tous_fournisseurs : Québec contre provinces + pays étrangers
    qui commercent avec l'état visé (nécessite Census)

Deux modes : preset Top25 (codes SH4 déterminés automatiquement depuis
isq_annuel.parquet) ou sélection personnalisée (sidebar, même style que
l'explorateur).
"""
...
with st.sidebar:
    ...
    with st.container(border=True):
        ...
        if mode == "Sélection personnalisée":
            ...
            st.caption("Les deux flux (DE et TI) sont calculés automatiquement en mode Top25.")
            flux_cochees = ["DE", "TI"]
    ...
    with st.container(border=True):
        ...
        options_etats = sorted(
            [noms_geo.get(c, c) for c, _ in entites_etats]
        )
        ...
        if mode == "Preset Top25":
            ...
            choix_etats = st.multiselect(
                "État(s)", options=options_etats, key="rc_etats_multi",
            )
    ...
    if mode == "Sélection personnalisée":
        with st.container(border=True):
            ...
            st.caption("Au moins un code SH2 ou SH4 requis — pas d'option "
                       "« tous les produits » pour cet outil (cohérent avec le script original).")

    # Devise LOCALE à cette page -- indépendante du sélecteur global de
    # l'explorateur, défaut USD (demandé explicitement, différent du défaut
    # "Native" de l'explorateur).
    with st.container(border=True):
...
if lancer:
    ...
    if erreurs:
        ...
        with st.status("Extraction en cours...", expanded=True) as statut:
            if mode == "Preset Top25":
                st.write("Détermination des 25 codes SH4 les plus importants (ISQ)...")
                codes_export = top25_sh4_isq(annees_selectionnees, "DE")
                codes_import = top25_sh4_isq(annees_selectionnees, "TI")
                codes_sh4_selectionnes = sorted(set(codes_export) | set(codes_import))
                st.write(f"{len(codes_sh4_selectionnes)} codes retenus.")
...
if df_affiche.empty:
    ...
    df_lisible["Province"] = df_lisible["origine"].map(lambda c: noms_geo.get(c, c))
    ...
    st.subheader(f"Résultats — {len(df_lisible):,} ligne(s)")
